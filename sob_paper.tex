...
%% ── Abstract ────────────────────────────────────────────────────────────────
\abstract{%
The Star of Bethlehem (Matthew~2:9) describes a celestial body guiding
travellers to a specific house and stopping above it.
Two independent linguistic analyses yield a joint Bayes factor of
${\approx}\,5.3\times10^8$ (conservative lower bound ${\approx}\,9\times10^7$)
favoring the legendary guiding-star tradition over astronomical discourse.
The external factor ($\mathrm{BF}_\text{corpus}\approx6{,}000$) reflects
Matthew's four-feature vocabulary (\grk{προάγω}, \grk{ἵστημι}, \grk{ἐπάνω},
\grk{ἔρχομαι}) being unattested across 25 surveyed Greek astronomical texts
(${\approx}\,622{,}000$ words, 4th~century~BCE to 4th~century~CE;
Fisher combined $p\approx6.3\times10^{-3}$).
The internal factor ($\mathrm{BF}_\text{Matthew}\approx88{,}000$) reflects
the same four words never appearing in astronomical senses across Matthew's
other 118 occurrences.
The datasets are disjoint and treated as conditionally independent,
as they derive from non-overlapping corpora and distinct sampling frames.
Orbital mechanics independently confirms: no Keplerian body can exhibit both
the sustained guidance and the punctual stopping the text requires, across
529~million tested orbital configurations.
The linguistic and mechanical evidence converge.}
...
$q \in \{0.02, 0.03, 0.04, 0.05, 0.07, 0.10, 0.15, 0.20, 0.25, 0.30,
...
\midrule
\textbf{Joint (A–D)} & — & — & — & $\mathbf{\chi^2(8)=21.4,\ p \approx 6.3\times10^{-3}}$ \\
\bottomrule
\end{tabular}
...
available at [repository URL]. Key scripts include:
