...
travellers southward for two hours, then halting above a specific house.
Rather than identifying the best candidate event, we derive kinematic constraints
from the text and show the feasible set of orbital solutions is empty.
A heliocentric survey of 529~million orbital configurations---spanning
elliptic, parabolic, and hyperbolic orbits across all inclinations,
eccentricities, perihelion distances, and orientations, tested at every
major proposed epoch from 12~BCE to 1~BCE---finds zero satisfying sustained
guidance motion ($>$2$^\circ$/h), punctual stopping ($<$2$^\circ$/h),
and the required azimuth corridor simultaneously.
Monte~Carlo refinement (2.25~million draws near the best-performing orbits),
geocentric surveys of 51{,}200 Earth-orbiting configurations, and a
close-flyby analysis of 10~million comet orbits confirm the null result.
The impossibility is kinematic: no heliocentric body can maintain the
required southward angular motion and subsequently halt above a fixed point.
...
absent from astronomical discourse across 25~Greek texts
(${\approx}\,622{,}000$~words; Fisher combined $p\approx6.3\times10^{-3}$),
while all four are characteristic of guiding-star legendary narratives
(joint Bayes factor ${\geq}\,10^7$ under maximally conservative assumptions;
baseline ${\approx}\,5.3\times10^{8}$).
Two independent lines of evidence converge: Matthew~2:9 is a legendary
narrative, not a record of an astronomical event.}
...
Here we show that such questions can instead be resolved by \emph{demonstration}:
by exhaustively surveying orbital parameter space and demonstrating that the
feasible set is empty.
We apply this paradigm to the most studied ancient astronomical claim
...
simultaneously\cite{adair2013star}, and the majority position in New Testament
criticism holds that Matthew's infancy narrative is legendary rather than
historical\cite{brown1993}, though active discussion remains within biblical studies (cf. \cite{barthel2015}). 
...
The linguistic datasets are disjoint; their joint product exceeds $10^7$
under maximally conservative assumptions (baseline ${\approx}\,5.3\times10^8$).
...
their Bayes factors are treated as conditionally independent.
The conservative lower bound on $\mathrm{BF}_\text{joint}$---obtained by pooling
omen texts with the guiding-star traditions and maximising inter-feature
correlation---is ${\approx}\,9.1\times10^7$; the baseline calculation gives
$87{,}696 \times 6{,}012 \approx 5.3\times10^8$.
...
other cultrually relevant details \cite{adair_jsnt}.

\paragraph{Broader applicability.}
The proof framework introduced here is generalizable beyond Matthew~2:9.
...
never arriving at a terrestrial geographic point.
...
evidence considered here (conservative lower bound ${\approx}9.1\times10^7$).
