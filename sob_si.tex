...
%% Engine-conditional Greek font setup
\ifPDFTeX
...
\title{\textbf{Supplementary Information}\\[6pt]
  \large Orbital mechanics and corpus linguistics rule out the Star of Bethlehem}
\author{Aaron M. Adair\\
...
I adopt $\omega_\mathrm{thresh} = 2^\circ\,\mathrm{h}^{-1}$ for both criteria.
Because the two rate quantities used in this paper are easily conflated, the
frame must be stated first.

\textit{Frame.} Matthew's claim is that the Star stood \emph{over a house}.
That is a statement about the object's bearing from a place on the ground, so
the criterion is evaluated in the horizon (alt-azimuth) frame, in which the
diurnal rotation of the Earth is present. This is the demanding frame: apparent
rest in it requires the object's parallactic motion to cancel the Earth's spin.
The \emph{inertial} rate (motion against the background stars, to which the
lunar figure of ${\approx}\,0.5^\circ\,\mathrm{h}^{-1}$ belongs) is a different
quantity, and is used in §\,S11 only where it is explicitly named.

\textit{Physical basis.} The threshold is anchored to the sky's own motion. For
the observer at Bethlehem, over the south-westerly part of the sky in which the
guidance corridor lies and above the $10^\circ$ altitude floor, a fixed star
moves in the horizon frame at between $11.6$ and $15.0^\circ\,\mathrm{h}^{-1}$,
depending on declination. Every star the Magi could see was drifting at that
rate. A stopping threshold is therefore a statement about what fraction of the
ambient sidereal rate still counts as standing still:
$2^\circ\,\mathrm{h}^{-1}$ admits an object drifting at roughly one part in six
to one in eight of the rate of the stars around it.

The lower bound is set by detectability rather than by the sky: naked-eye
angular resolution is ${\approx}\,1'$, so motion is detectable above
${\approx}\,0.03^\circ\,\mathrm{h}^{-1}$ over a thirty-minute window, and no
threshold below that could be observationally meaningful. The choice of
$2^\circ\,\mathrm{h}^{-1}$ is deliberately generous at the upper end; any
stricter threshold only strengthens the conclusion.

\paragraph{What would overturn the result.}
Because a negative result of this kind can always be attacked by loosening the
criterion that produced it, I state the falsification condition in advance and
in physical units rather than leaving it to be negotiated.

The tightest value achieved by any orbit, across $10^9$ samples and every
parameter combination tested, is $6.94^\circ\,\mathrm{h}^{-1}$ (§\,\ref{subsec:sweep}). That
is between $46\%$ and $60\%$ of the local sidereal rate, depending on
declination. Overturning the conclusion therefore requires adopting a stopping
threshold above that figure, and the burden on any such threshold is to explain
in what sense an object crossing the sky at half the speed of the stars beside
it can be said to stand still over a particular house. Over the two hours of
the journey described, such an object would traverse some $14^\circ$, about
twenty-seven lunar diameters, or a hand's breadth and a half at arm's length.

The threshold is thus not a free parameter to be widened until a candidate
appears. It is bounded below by naked-eye detectability and above by the motion
of the sky itself, and the result reported here fails at every value between
those bounds. The figure that would have to be conceded is
$6.94^\circ\,\mathrm{h}^{-1}$; the argument that it cannot be conceded is given
above, and is offered for the reader to test rather than asserted.
...
\begin{enumerate}
  ...
  \item \textit{Motion (\grk{προάγω}):} minimum apparent angular velocity
    ...
    ``go before'' travelers; this criterion excludes such configurations.
\end{enumerate}
...
\begin{table}[h]
  ...
  \begin{tablenotes}
    ...
    \item The upper end of the sweep ($10^\circ\,\mathrm{h}^{-1}$) exceeds any physically defensible stopping threshold. In the horizon frame in which the criterion is evaluated (§\,2.3), a fixed star over Bethlehem moves at $11.6$--$15.0^\circ\,\mathrm{h}^{-1}$ in the relevant part of the sky. A stopping criterion of $10^\circ\,\mathrm{h}^{-1}$ would therefore count an object as standing still while it drifted at roughly two-thirds to nine-tenths of the rate of every star around it, which is not a description any observer would give. The sweep accordingly covers every physically coherent criterion value at both ends: below by the limit of naked-eye detection, above by the ambient sidereal rate.
  \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{table}[h]
  ...
  \centering
  \small
  \begin{tabular}{@{}lrrrp{2.8cm}p{3.2cm}@{}}
    \toprule
...
that ``Keplerian orbits cannot decelerate abruptly over four hours''; I
...
second law). Although $h$ is conserved about the \emph{solar} focus while the
observer is at \emph{Earth}, the correction is of order $r_E/d$ and is already
...
geometric transit artifact. Combined with the heliocentric result, no orbit type satisfies the combined guidance, stopping, and azimuth criteria of Matthew~2:9.
...
a created angelic messenger, not a heavenly body. Gregory of Nyssa similarly emphasized the Star's
...
object (Adair 2012); but some recent efforts have nonetheless tried to renew the debate 
within biblical studies \cite{barthel2015}.
...
across 24 astronomical and astrological texts totalling approximately 619,000 words, \grk{ἀνατολή}
...
carry specialized astrological meanings.
...
as a technical heliacal-rising term across the entire 24-text corpus.
...
reaching its synodic stationary point. Greek astronomical writers do use \grk{ἵστημι}
of a planet ceasing its motion. Geminus (\textit{Elem.\ astr.}~1.20) glosses planets
``standing still'' (\grk{ἑστηκέναι}) as what ``they call \grk{στηριγμούς}''; Theon of
Smyrna (p.\,148) defines \grk{στηριγμός} as the appearance of a planet \emph{standing}
(\grk{ἑστῶτος}) and remaining beside one of the fixed stars; Paul of Alexandria (p.\,31)
derives \grk{στηριγμός} from the stars \emph{standing still} (\grk{ἑστάναι}); and
Hipparchus (\textit{Frag.\ geogr.}~7) glosses \grk{μοναί} as \grk{ἵστασθαι}. The verb
was available for the sense Molnar requires.

What the corpus withholds is not the verb but its complement. In all 121 in-corpus
occurrences of \grk{ἵστημι}, what a celestial body stands in, on, or above is a zodiacal
sign, a degree, an aspect, a chart angle, or a fixed star. It is never a house, a city,
or a person. The Molnar reading therefore fails on two grounds. First, the \grk{ἕως}
construction (§\,S2.2) marks \grk{ἐστάθη}
as a punctual change of state from prior motion, which is not what a planetary station
represents: a planet approaching its station decelerates continuously over weeks, rather
than moving and then stopping abruptly. Second, a planetary station is a phenomenon
defined against the stellar background, whereas \grk{ἐστάθη ἐπάνω} in
context describes a position fixed above a terrestrial location. The reading requires a
grammatical and a cosmological mapping that the sources do not support.
...
individual philological observations: across 24 ancient Greek astronomical and astrological
...
rates yielding a joint Bayes factor of ${\approx}3.0\times10^{6}$ in favor of the legendary
...
Theodosius       & \textit{On Habitations} (Περὶ οἰκήσεων)    & \textsc{ii} bce &   2,800 & Math.~astr. \\
\midrule
\multicolumn{3}{l}{\textbf{Total (24 texts)}} & \textbf{$\approx$619,000} & \\
\bottomrule
\end{tabular}
\smallskip\par
...
the corpus was searched by exact wordform over the full paradigm of the target
lemma, one form at a time, with diacritic-sensitive matching and full results
paging. Feature~E (\grk{ὁδηγέω}/\grk{ἡγέομαι}) was removed
...
\begin{enumerate}
  ...
  \item \textit{Semantic}: does the usage match the specific sense found in
    Matthew~2:9 (spatial guidance of human travelers; spatial arrival of a
...
All passages retrieved by the wordform searches were individually read in
context;
...
\begin{itemize}
  \item Feature A: \grk{προάγω}: all inflected forms searched individually
    as exact wordforms; classified for spatial/narrative guidance of a
    person toward a destination versus purely positional/temporal
    antecedence.
  \item Feature B: \grk{ἵστημι}: every inflected form searched
    individually as an exact wordform (164 forms of the paradigm tested,
    69 attested); classified for location of a celestial body above a
    named terrestrial point versus all other senses.
  \item Feature C: \grk{ἐπάνω} and \grk{ὑπὲρ}: all positional prepositional
    ...
    \grk{Ἐπάνω} is indeclinable, so a single exact search retrieves it.
  \item Feature D: \grk{ἔρχομαι}: every inflected form of all four
    suppletive stems (\grk{ἐρχ-}, \grk{ἐλευσ-}, \grk{ἐλθ-/ἠλθ-},
    \grk{ἐληλυθ-}) searched individually as exact wordforms (171 forms
    tested, 61 attested); classified for arrival at a terrestrial
    geographic point versus other spatial or temporal senses.
\end{itemize}

\noindent Every feature is searched by exact wordform, one inflected form at a
time, with \textsf{Exact Match} and \textsf{Diacritics sensitive} both enabled
and results-per-page set above the largest expected hit count. Forms are
enumerated from a printed paradigm rather than generated by the interface: the
TLG ``Lemma'' mode matches wordforms that begin with the headword string, so a
query on \grk{λέγω} returns \grk{λέγων} and \grk{λέγωσι} but not \grk{λέγει} or
\grk{ἔλεγεν}, and it is unsuitable for counting a lemma's occurrences. For
authors represented in the corpus by a single work, hits are filtered by TLG
work number (Aristotle \{0086.026\}, Theophrastus \{0093.008\}). The procedure
is reproducible by any reader with TLG access, and the passage-level record in
Table~S8 allows every classification to be checked independently.
...
Table~\ref{tab:tlg_hits} records every passage retrieved by the wordform searches
...
\caption{Complete TLG corpus hit record for features A--D
  (TLG searches conducted via MIT library proxy, 2026-09-04, by exact wordform
  search over the full paradigm of each lemma.)
  \textit{Matthew sense:}
  A~=~star guides travelers to a destination;
...
%%--------------------------------------------------------------------------
\multicolumn{5}{@{}p{\linewidth}@{}}{\textbf{Feature A: \grk{προάγω} (exact wordform search).}
  \textit{Matthew sense}: star actively leads/guides human travelers toward a
...
Vettius Valens \{1764\} & 4 &
  ...
  degree-order or timing); no human traveler, no destination. & Neg \\[3pt]
Hephaestion \{2043\} & 3 &
...
%%--------------------------------------------------------------------------
\multicolumn{5}{@{}p{\linewidth}@{}}{\textbf{Feature B: \grk{ἵστημι} (exact wordform search, full paradigm).}
  \textit{Matthew sense}: celestial body comes to rest above a specific
  terrestrial location. Every one of the 121 in-corpus occurrences was read
  in context and classified on the same criterion applied to the other three
  features: does the passage locate a celestial body above a named
  terrestrial point? All 121 are negative. Counts by author follow; the
  passage-level record, with locations and sense assignments, is given in
  Table~S8b.} \\[3pt]
%%--------------------------------------------------------------------------
Ptolemy \{0363\} & 30 &
  \textit{Alm.}, \textit{Harm.}, \textit{Mus.},
  \textit{Tetrab.}, \textit{Inscr.\ Canobi} &
  Instruments set upright; the earth at rest; the ``fixed notes''
  (\grk{φθόγγοι ἑστῶτες}) of the \textit{Harmonica}; planets standing in
  aspect. No terrestrial reference. & Neg \\[3pt]
Vettius Valens \{1764\} & 13 &
  \textit{Anth.}~1.1, 2.17, 2.30, 2.32, 6.4, 6.9, 9.8;
  \textit{Frag.} &
  The zodiacal sign ``in which the Moon (or Sun, or Venus) stands''
  (\grk{ἐφ᾽ οὗ ἕστηκεν}); setting the Moon upon a degree; the sea seeming
  to stand. Complement always a sign or degree. & Neg \\[3pt]
Dorotheus \{1337\} & 11 &
  \textit{Frag.\ Graeca}: pp.\,325, 336, 339, 341, 345, 350, 364, 372, 389 &
  Planets standing in good or foreign chart places (\grk{τόποι}); casting
  the horoskopos. Zodiacal, not terrestrial. & Neg \\[3pt]
Hephaestion \{2043\} & 11 &
  \textit{Apot.}~1: pp.\,39, 90, 118, 181, 188, 198, 227, 273;
  \textit{Apot.}~2: pp.\,80, 81 &
  Lords standing well in chart places; setting the angles; Mars set square
  or diametric to the luminaries. & Neg \\[3pt]
Achilles Tatius \{2133\} & 10 &
  \textit{Isagoga}~§§4, 9, 19, 29 &
  Whether the earth and the cosmos stand still; the Moon standing between
  earth and sun in eclipse; a person's shadow. & Neg \\[3pt]
Cleomedes \{1272\} & 8 &
  \textit{Caelestia}~1.1; 2.1, 2.5, 2.6 &
  The earth at rest; observers standing on the earth; the sun
  \grk{στῇ} at exact noon while Syene lies \emph{under Cancer}
  (2.1.271, the nearest approach in the corpus, and still not a
  celestial body standing above a named place: Syene is tied to Cancer by
  \grk{κεῖται}, not to the sun by \grk{ἵστημι}). & Neg \\[3pt]
Theon of Smyrna \{1724\} & 6 &
  \textit{De util.\ math.}~pp.\,122, 132, 139, 148, 151, 191 &
  The definition of \grk{στηριγμός} (p.\,148); a planet seeming to stand
  and go backwards (p.\,191); the zodiac standing \grk{ὑπὲρ ἡμᾶς}
  (p.\,132); a person standing on a shore. & Neg \\[3pt]
Manetho \{2583\} & 6 &
  \textit{Apot.}~1.344; 2.25, 2.107; 3.245; 6.31, 6.287 &
  Planets standing in aspect or \grk{ὑπὲρ κέντρου μεσάτοιο} (above the
  chart's midmost angle); the poles standing unmoved. & Neg \\[3pt]
Eratosthenes \{0222\} & 6 &
  \textit{Catast.}~1.4, 1.6D, 1.10, 1.11, 1.26, 1.28 &
  Constellation figures standing (Heracles, Aquarius, Sagittarius); Zeus
  setting the Twins among the stars. & Neg \\[3pt]
Theodosius \{1719\} & 4 &
  \textit{Sphaerica}~1.7; \textit{De habitationibus}~§3 &
  The zodiac circle standing upright; a straight line set upon a straight
  line (Euclidean perpendicular). Geometric. & Neg \\[3pt]
Paul of Alexandria \{2053\} & 4 &
  \textit{Elem.\ apot.}~pp.\,31 ($\times$2), 75, 76 &
  Derives \grk{στηριγμός} from the stars \emph{standing still}
  (\grk{ἑστάναι}); planets standing upon chart places. & Neg \\[3pt]
Aratus \{0653\} & 4 &
  \textit{Phaen.}~306, 610, 865 &
  Scorpius standing before Sagittarius; Argo standing aloft; the Moon
  standing between earth and sun. Celestial positions only. & Neg \\[3pt]
Hipparchus \{1431\} & 3 &
  \textit{In Arat.}~1.7.1, 2.2.42; \textit{Frag.\ geogr.}~7 &
  Quotations of Aratus; \grk{μοναί} (stations) glossed as
  \grk{ἵστασθαι}. & Neg \\[3pt]
Geminus \{1383\} & 2 &
  \textit{Elem.\ astr.}~1.20; 17.9 &
  Planets ``standing still'' (\grk{ἑστηκέναι}), which ``they call
  \grk{στηριγμούς}''; fixed signs marking weather changes. & Neg \\[3pt]
Aristotle (\textit{Meteor.}) \{0086.026\} & 2 &
  Bekker 374b\,4; 376b\,27 &
  A person standing outside as the rainbow appears; the rainbow standing
  where the sun is most overcome. & Neg \\[3pt]
Theophrastus (\textit{De signis}) \{0093.008\} & 1 &
  §12~l.56 &
  Snakes standing and quivering. Non-celestial. & Neg \\[3pt]
Aristarchus, Autolycus,
  Hypsicles & 0 each &
  --- & No instances of \grk{ἵστημι} in any of these corpus texts. & Neg \\[6pt]

%%--------------------------------------------------------------------------
\multicolumn{5}{@{}p{\linewidth}@{}}{\textbf{Feature C: \grk{ἐπάνω} (exact wordform search; the word is indeclinable).}
  ...
  above the horizon or another body. All 40 corpus hits use geometric or
  astrological ``above'' without terrestrial address.} \\[3pt]
%%--------------------------------------------------------------------------
Ptolemy \{0363\} & 14 &
  \textit{Alm.}~vol.1,1 p.475~l.3;
  vol.1,2 pp.40--41, 44--45 ($\times$2),
  62--63, 64--65, 152--153, 267, 268,
  289, 495, 578;
  \textit{Tetrab.}~3.4.5.7;
  \textit{Harm.}~3.2.23 &
...
Theon of Smyrna \{1724\} & 4 &
  \textit{De util.\ math.}~pp.\,34~l.9, 75~l.8,
  148~l.9, 201~l.11 &
  Geometric and arithmetical ``above'' in the exposition of ratios and
  planetary order; no terrestrial address. & Neg \\[3pt]
Aristotle (\textit{Meteor.}) \{0086.026\} & 3 &
  Bekker 375b\,28; 377a\,4; 377a\,6 &
  Geometric ``above'' in the analysis of the rainbow and halo; no
  celestial body localized above a place. & Neg \\[3pt]
Theophrastus \{0093\} & 0 in corpus &
  (13 hits author-wide, none in
  \textit{De signis}) &
  All Theophrastus \grk{ἐπάνω} hits are in works 002, 004, 005, 010, 014,
  017 (not \textit{De signis}); 0 in-corpus hits. & Neg \\[3pt]
Cleomedes, Hipparchus, Geminus,
...
%%--------------------------------------------------------------------------
\multicolumn{5}{@{}p{\linewidth}@{}}{\textbf{Feature D: \grk{ἔρχομαι} (exact wordform search, all four suppletive stems).}
  \textit{Matthew sense}: a celestial body arrives at a specific terrestrial
  location (a house, a person's location). \grk{Ἔρχομαι} is suppletive; the
  stems \grk{ἐρχ-}, \grk{ἐλευσ-}, \grk{ἐλθ-/ἠλθ-} and \grk{ἐληλυθ-} were
  searched separately (171 forms tested, 61 attested), including the aorist
  that supplies Matthew's own \grk{ἐλθών}. Of the 356 in-corpus occurrences,
  103 have a celestial subject. The construction is standard and frequent
  (\grk{ὁ Ἄρης ἐπὶ τὸν Ἥλιον ἐλθών}, ``Mars coming upon the Sun'';
  \grk{ὁ Ζεὺς ἐλθὼν εἰς τὸν ὡροσκόπον}, ``Jupiter coming into the
  horoskopos''), but the destination is invariably another body or a point
  in the chart. Tabulating the objects governed by
  \grk{ἐπί}/\grk{εἰς}/\grk{πρός} across those 103 passages yields
  \grk{ὡροσκόπον}, \grk{Δία}, \grk{Ἥλιον}, \grk{Κρόνον}, \grk{ζῴδιον},
  \grk{μοῖραν}, \grk{κέντρα}, \grk{μεσουράνημα}, \grk{ὑπόγειον},
  \grk{τόπους}, \grk{διάμετρον}, \grk{τετράγωνα}, and no terrestrial place
  whatever.} \\[3pt]
%%--------------------------------------------------------------------------
Vettius Valens \{1764\} & 98 &
  \textit{Anthol.}~and \textit{Frag.}, passim &
  Planet ``comes to'' (\grk{ἐλθών}) another planet, the horoskopos, a
  degree, sign or aspect; the destination is a zodiacal or chart locus,
  never a terrestrial address. & Neg \\[3pt]
Hephaestion \{2043\} & 65 &
  \textit{Apot.}~1 and \textit{Apot.}~2, passim &
  Same usage throughout. & Neg \\[3pt]
Dorotheus \{1337\} & 42 &
  \textit{Frag.\ Graeca}, passim &
  Same usage; e.g.\ p.\,379~l.22 \grk{ὁ Ἄρης ἐπὶ τὸν Ἥλιον ἐλθών}. & Neg \\[3pt]
Ptolemy \{0363\} & 28 &
  \textit{Alm.}, \textit{Tetrab.}, \textit{Harm.} &
  Arrival at a computed position, a season, or a stage of an argument;
  no terrestrial destination for a celestial body. & Neg \\[3pt]
Theodosius \{1719\} & 24 &
  \textit{Sphaerica}, \textit{De hab.}, \textit{De diebus} &
  Geometric: a point or arc ``arriving at'' another in a construction. & Neg \\[3pt]
Manetho \{2583\} & 19 &
  \textit{Apot.}, passim &
  Planets arriving in aspect or sign. & Neg \\[3pt]
Aratus \{0653\} & 18 &
  \textit{Phaen.}, passim &
  Risings and settings; constellations coming round in the annual cycle. & Neg \\[3pt]
Aristotle (\textit{Meteor.}) \{0086.026\} & 15 &
  Bekker, passim &
  Meteorological and argumentative senses; no celestial body at a place. & Neg \\[3pt]
Theon of Smyrna \{1724\} & 14 &
  \textit{De util.\ math.}, passim &
  Includes p.\,154~l.6, the sole passage in the corpus contemplating a
  celestial body arriving at the earth, and it is a \emph{reductio}:
  were the solar circle to pass through that point, ``the sun itself would
  come to the earth'', which the author calls \grk{ἀμήχανον}, impossible. & Neg \\[3pt]
Eratosthenes \{0222\} & 14 &
  \textit{Catast.}, passim &
  Mythological narrative of the catasterism, and stars rising. & Neg \\[3pt]
Autolycus \{1210\} & 6 &
  \textit{De sphaera}, \textit{De ortibus} &
  Geometric arrival of a point on a circle. & Neg \\[3pt]
Hipparchus \{1431\} & 5 &
  \textit{In Arat.} &
  Commentary on risings; no terrestrial destination. & Neg \\[3pt]
Cleomedes \{1272\} & 3 &
  \textit{Caelestia} &
  Light and observers; no celestial body at a named place. & Neg \\[3pt]
Theophrastus (\textit{De signis}) \{0093.008\} & 3 &
  §§\,passim &
  Weather signs; non-celestial subjects. & Neg \\[3pt]
Geminus \{1383\} & 1 &
  \textit{Elem.\ astr.} &
  Positional arrival in the zodiac. & Neg \\[3pt]
Achilles Tatius \{2133\} & 1 &
  \textit{Isagoga} &
  Cosmographic exposition. & Neg \\[3pt]
Aristarchus, Hypsicles,
  Paul of Alexandria & 0 each &
  --- & No instances of \grk{ἔρχομαι} in any of these corpus texts. & Neg \\[3pt]
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}p{0.5cm}p{3.8cm}p{3.8cm}p{2.2cm}p{2.2cm}@{}}
    ...
    A & \grk{προάγω} with celestial subject in narrative guidance of
        human travelers toward a destination &
      Celestial body actively directs human travelers toward a specific
        destination &
  ...
  lexical analysis (BF~$\approx 3.0\times10^{6}$) therefore also characterizes the
...
\textit{Feature A (guidance of travelers).}
...
function, a celestial body leading travelers step by step toward a goal, is
...
that a body ``stands over'' a house, city, or person.

Matthew's own verb, \grk{ἵστημι}, occurs 121 times in the corpus, and none
of the 121 carries the Matthean sense. The distribution of senses is
instructive: a planet or luminary standing \emph{in} a zodiacal sign, degree
or chart place (roughly 45 occurrences); the ``fixed notes''
(\grk{φθόγγοι ἑστῶτες}) of Ptolemy's \textit{Harmonica} (about 20); the
earth, cosmos or sea at rest, and eclipse geometry (about 15); constellation
figures standing: Heracles, Aquarius, Sagittarius, Virgo (about 8);
instruments set upright and geometric lines set upon lines (about 10);
persons standing (about 10); the transitive casting of a chart, ``to set the
horoskopos'' or ``the angles'' (about 8); and the planetary station itself
(about 5). In every case the complement of the standing is celestial or
geometric (a sign, a degree, an aspect, a chart angle, a fixed star) and
in no case is it a named place on the ground. Matthew's own form
\grk{ἐστάθη} does not occur in the corpus at all.
...
Arrival language in the corpus is abundant. \grk{Ἔρχομαι} occurs 356 times,
103 of them with a celestial subject, and the construction Matthew uses (a
body arriving somewhere, expressed with \grk{ἐπί}, \grk{εἰς} or \grk{πρός}) is
the ordinary idiom of the astrological writers: \grk{ὁ Ἄρης ἐπὶ τὸν Ἥλιον
ἐλθών}, ``Mars coming upon the Sun'' (Dorotheus p.\,379~l.22); \grk{ὁ Ζεὺς
ἐλθὼν εἰς τὸν ὡροσκόπον}, ``Jupiter coming into the horoskopos'' (Vettius
Valens \textit{Frag.}~vol.\,8,1 p.\,165~l.5). The feature is therefore not
absent for want of the verb or of the construction.

What the 103 celestial-subject passages never supply is a terrestrial
destination. Tabulating every object governed by \grk{ἐπί}, \grk{εἰς} or
\grk{πρός} in those passages returns \grk{ὡροσκόπον}, \grk{Δία},
\grk{Ἥλιον}, \grk{Κρόνον}, \grk{Ἀφροδίτην}, \grk{ζῴδιον}, \grk{μοῖραν},
\grk{κέντρα}, \grk{μεσουράνημα}, \grk{ὑπόγειον}, \grk{τόπους},
\grk{διάμετρον} and \grk{τετράγωνα}: other bodies, and points within the
chart. Not one is a place on the ground.

\paragraph{The single counter-case, and what it says.}

Exactly one passage in the corpus puts a celestial body and the earth
together as arrival and destination. Theon of Smyrna, discussing whether the
solar circle could pass through a particular point of his construction,
writes (\textit{De util.\ math.}~p.\,154~ll.\,5--7):

\begin{quote}
\grk{διὰ μὲν οὖν τοῦ θ τὸν ἡλιακὸν ἔρχεσθαι κύκλον, ἀμήχανον· καὶ γὰρ αὐτὸς
ἂν ὁ ἥλιος ἐπὶ γῆν παρεγίνετο}

``For the solar circle to travel through \grk{θ} is impossible; for then the
sun itself would come to the earth.''
\end{quote}

The sentence is a \emph{reductio ad absurdum}. Theon raises the possibility
of a celestial body arriving at the earth in order to reject it, and marks it
\grk{ἀμήχανον}, not merely unattested but impossible. The consequent is put
in the past contrafactual (\grk{ἂν \dots\ παρεγίνετο}), the standard Greek
construction for an outcome that does not and cannot obtain.

This is the strongest single datum in the corpus analysis. The claim under
test is that Greek astronomical writers do not describe celestial bodies
arriving at terrestrial locations. The corpus supplies 356 occurrences of the
relevant verb without a single such description; and where one author does
frame the notion explicitly, he frames it as an absurdity used to refute a
geometric hypothesis. The absence is not a gap in the sample. It is a
commitment of the discourse.
...
guided-traveler narratives from the entire corpus; and a critic who argued
...
$p_A = 0.083$ ($N=11$), $p_B = 0.0082$ ($N=121$), $p_C = 0.024$ ($N=40$),
$p_D = 0.0028$ ($N=356$)
(TLG counts, 2026-09-04, by exact wordform search over the full paradigm of
each lemma, with \textsf{Exact Match} and \textsf{Diacritics sensitive}
enabled; see Table~S8.)
The Fisher combined chi-squared statistic is
$X = -2\sum_i \ln p_i = 33.76$ on $\mathrm{df}=8$, giving
$p_\text{Fisher} \approx 4.5\times10^{-5}$.
...
$P(A,B,C,D \mid H_\text{astro}) = \prod_i p_i = 4.7\times10^{-8}$.

Features~B and~D individually pass the Bonferroni threshold
$\alpha/4 = 0.013$; Features~A and~C do not, and A remains at the floor value
imposed by its corpus size ($1/12$, the smallest value that denominator
admits). The joint Fisher combined statistic is significant
($p = 4.5\times10^{-5}$) and all four features contribute under the joint
test.
...
Matthew-like words simultaneously. To assess this, I modeled the joint
...
$X = -2\sum_i \ln p_i$ was computed. The observed value $X = 33.76$
was compared against the simulated null distribution at each
$\rho$ using $2\times10^6$ Monte Carlo draws.

The corrected joint $p$ remains below $\alpha = 0.05$ across the entire
tested range, to $\rho = 0.95$ (Table~\ref{tab:copula}); at $\rho=0.5$ the
corrected $p = 0.0027$. Brown's variance-corrected chi-squared
...
\midrule
0.0  & $4.3\times10^{-5}$ & $4.5\times10^{-5}$ \\
0.2  & $2.6\times10^{-4}$ & $9.8\times10^{-5}$ \\
0.4  & $1.5\times10^{-3}$ & $5.0\times10^{-4}$ \\
0.5  & $2.7\times10^{-3}$ & $1.1\times10^{-3}$ \\
0.6  & $4.3\times10^{-3}$ & $2.3\times10^{-3}$ \\
0.7  & $6.4\times10^{-3}$ & $4.2\times10^{-3}$ \\
0.8  & $8.9\times10^{-3}$ & $6.9\times10^{-3}$ \\
0.9  & $1.2\times10^{-2}$ & $1.0\times10^{-2}$ \\
0.95 & $1.3\times10^{-2}$ & $1.2\times10^{-2}$ \\
\bottomrule
\end{tabular}
  \begin{tablenotes}
    \footnotesize
    ...
    all four grammatically distinct feature classes. The corrected $p$ remains
    below $\alpha = 0.05$ throughout the tested range, including at
    $\rho = 0.95$, so the conclusion does not depend on any judgement about
    the plausible magnitude of $\rho$.
...
drop-one: $p = 0.0012$, dropping Feature~D). Every three-feature subset also
remains significant; the weakest yields $p = 0.015$ (dropping Features B and D
together), which is
...
\midrule
\endfirsthead
\multicolumn{5}{@{}p{\linewidth}@{}}{\footnotesize\textit{Table~\thetable\ continued}}\\[2pt]
\toprule
Tradition (constituent texts) & A & B & C & D \\
\midrule
\endhead
...
%%
%% -- Primary corpus --------------------------------------------------------
\multicolumn{5}{@{}p{\linewidth}@{}}{\textit{Primary corpus: four independent guiding-star traditions ($N=4$)}} \\
\midrule
...
%% -- Omen texts ------------------------------------------------------------
\midrule
\multicolumn{5}{@{}p{\linewidth}@{}}{\textit{Stellar-omen texts (sensitivity analysis only; excluded from primary corpus)}} \\
\midrule
...
%% -- Matthean derivative ---------------------------------------------------
\midrule
\multicolumn{5}{@{}p{\linewidth}@{}}{\textit{Matthean derivative (excluded)}} \\
\midrule
...
\midrule
\multicolumn{5}{@{}p{\linewidth}@{}}{\textit{Supernatural descent texts (feature~C commentary only; not in BF corpora)}} \\
\midrule
PGM~I.54--95
  & 0 & 1 & 0 & 1 \\
...
\multicolumn{5}{@{}p{\linewidth}@{}}{\footnotesize
  ...
  BF~$= 1.39\times10^{-1}/4.67\times10^{-8} \approx 3.0\times10^{6}$ (exceeds 100 on the Jeffreys scale).
  $P(A,B,C,D\mid H_\text{astro}) = \prod_i p_i = 4.67\times10^{-8}$
  (product of individual Fisher exact probabilities; the Fisher combined tail-$p$
  of $4.5\times10^{-5}$ is a meta-test statistic, not a joint data probability).
  Sensitivity: $+$Josephus \& Dio ($N=6$): BF~$=1.3\times10^{6}$; $+$all omen ($N=8$): BF~$=5.1\times10^{5}$;
  $k\in[0.5,3]$: BF~$\in 2.3$--$3.4\times10^{6}$; $3\times$ denominator uncertainty: $1.0$--$9.3\times10^{6}$.
...
\begin{itemize}
  ...
  \item \texttt{Stellarium}\cite{stellarium2021}, independent cross-validation
    of sky-position results at representative epochs; computed positions
    agreed to within observational tolerances at all tested times.
  \item \texttt{VSOP87} (via \texttt{PyMeeus}): Earth's heliocentric position
    for all close-approach geometry.  The bundled modern DE ephemerides do not
    extend to 5~BCE, and extrapolating the low-precision default series to that
    epoch introduces an error of ${\approx}43''$ in Earth's position.  For
    objects at ordinary heliocentric distances this is immaterial, but at a
    geocentric distance of ${\sim}10^{5}$\,km it displaces the observer by
    ${\approx}3\times10^{4}$\,km and swings the sightline by degrees, so the
    close-flyby analyses use VSOP87 throughout.
\end{itemize}

\subsection{Reproducibility, and the tolerance on configuration counts}
\label{subsec:reproducibility}

Two kinds of number are reported from the surveys, and they carry different
reproducibility.

The \emph{result}, the number of configurations satisfying all criteria
simultaneously, is exact and environment-independent.  It is zero, and reruns
under different software builds return zero identically.  The extremal
diagnostics are likewise reproducible bit-for-bit: best guidance score
$0.0414^\circ$ and best stopping score $0.0376^\circ\,\mathrm{h}^{-1}$ agree to
sixteen significant figures across environments.  This is expected, since the
nearest candidate misses by a wide margin and no plausible numerical
perturbation can move it across the criterion.

The \emph{census} counts (how many configurations are classed as visible, or
as passing one criterion in isolation) carry a small systematic tolerance.
Each is a threshold test, so configurations lying within numerical noise of a
boundary may fall on either side of it depending on the ephemeris
implementation.  Re-running the fine survey under a different
\texttt{astropy}/\texttt{pyerfa} build gives:

\begin{center}
\small
\begin{tabular}{@{}lrr@{}}
\toprule
Census bin & Count & Tolerance \\
\midrule
Visible                       & 202{,}372{,}076 & $\pm 0.1\%$ \\
Passing azimuth criterion     &     126{,}720   & $\pm 0.3\%$ \\
Passing guidance-motion criterion &   6{,}278   & $\pm 0.7\%$ \\
Passing stopping criterion    & 201{,}505{,}224 & $\pm 0.1\%$ \\
\midrule
\textbf{Satisfying all criteria} & \textbf{0} & \textbf{exact} \\
\bottomrule
\end{tabular}
\end{center}

The tolerance is larger for the smaller bins simply because a fixed number of
boundary configurations is a larger fraction of a smaller count.  These figures
should therefore be read as $2.024\times10^{8}$ visible configurations to within
a tenth of a percent, not as exact integers; the counts quoted throughout were
produced with Python~3.12, \texttt{astropy}~6.x and the bundled
\texttt{pyerfa}.  Only the final row is exact, and it is the only row on which
the paper's conclusion depends.

Code and data necessary to reproduce all figures and tables in the main
paper and this Supplementary Information are available at \url{https://github.com/adairaar/sob_astro_test}.
...
with a Fisher combined $p \approx 4.5\times10^{-5}$ against the null hypothesis
...
Bayes factor in favor of the narrative reading is $\mathrm{BF} \approx 3.0\times10^{6}$
(exceeding 100 on the Jeffreys scale) under strict independence, and
$5.1\times10^{5}$ if guiding and omen texts are pooled. Sensitivity to the
dependence assumptions is treated separately in
Table~\ref{tab:bfsens}, which is the range on which the conclusion rests.
...
guiding human travelers toward a destination, ceasing motion above a
...
travelers (features A, D), and neither enters the Bayes factor corpora. They
...
The corpus Bayes factor ($\mathrm{BF}_\text{corpus} \approx 3.0\times10^{6}$) tests whether
...
\paragraph{Sensitivity of the Bayes factor to its independence assumptions.}

Both Bayes factors are products over the same four features, and two
independence assumptions are embedded in that product. Neither is exactly
true, and the figures are therefore reported as a range rather than as a point
estimate (Table~\ref{tab:bfsens}).

\emph{Within a feature} ($\rho_w$). The estimate $p = 1/(N+1)$ is a rule of
succession over $N$ trials with no successes, which is exact only if the $N$
occurrences are independent opportunities. They are clustered by author: 98 of
the 356 occurrences of \grk{ἔρχομαι} are Vettius Valens. Effective sample sizes
are therefore computed with the Kish design effect for unequal clusters,
$N_\mathrm{eff} = N / \left(1 + (\bar m - 1)\rho_w\right)$, where $\bar m$ is
the size-weighted mean cluster size.

Against this, the absence is replicated across sixteen authors, several
centuries, and both mathematical astronomy and astrology. Were it an idiolect,
between-author heterogeneity would be expected; none is observed, every author
standing at zero. That uniformity is evidence that the constraint operates at
the level of the register rather than the individual, and it bounds $\rho_w$
from above. The tabulated range accordingly stops at $0.2$.

\emph{Between features} ($\rho_b$). The four features are four properties of a
single clause. This is already conceded above, and corrected for in the Fisher
combination by the Gaussian copula of Table~\ref{tab:copula}. The same
correction is applied here to the Bayes factor: the effective feature count is
taken as $k_\mathrm{eff} = \mathrm{df}_\mathrm{Brown}/2$ and the
independence product is downweighted geometrically to
$\mathrm{BF}^{\,k_\mathrm{eff}/4}$. This is a heuristic rather than an exact
result, and is reported as one.

\begin{table}[h]
\centering
\begin{threeparttable}
  \caption{Sensitivity of the Bayes factor and the Fisher combination to
    within-feature clustering ($\rho_w$) and between-feature dependence
    ($\rho_b$).}\label{tab:bfsens}
\begin{tabular}{@{}ccccc@{}}
\toprule
$\rho_w$ & $\rho_b$ & $\mathrm{BF}_\text{corpus}$ &
  $\mathrm{BF}_\text{joint}$ & Fisher $p$ \\
\midrule
0.0 & 0.0 & $3.0\times10^{6}$ & $2.6\times10^{11}$ & $4.5\times10^{-5}$ \\
0.0 & 0.4 & $2.6\times10^{4}$ & $6.0\times10^{7}$  & $5.0\times10^{-4}$ \\
0.0 & 0.5 & $5.5\times10^{3}$ & $3.9\times10^{6}$  & $1.1\times10^{-3}$ \\
0.0 & 0.7 & $4.4\times10^{2}$ & $4.5\times10^{4}$  & $4.2\times10^{-3}$ \\
\midrule
0.1 & 0.0 & $1.1\times10^{5}$ & $9.9\times10^{9}$  & $6.5\times10^{-4}$ \\
0.1 & 0.4 & $2.8\times10^{3}$ & $6.4\times10^{6}$  & $3.3\times10^{-3}$ \\
0.1 & 0.5 & $8.3\times10^{2}$ & $5.9\times10^{5}$  & $5.8\times10^{-3}$ \\
0.1 & 0.7 & $1.1\times10^{2}$ & $1.2\times10^{4}$  & $1.4\times10^{-2}$ \\
\midrule
0.2 & 0.0 & $2.5\times10^{4}$ & $2.2\times10^{9}$  & $2.1\times10^{-3}$ \\
0.2 & 0.4 & $1.0\times10^{3}$ & $2.3\times10^{6}$  & $7.8\times10^{-3}$ \\
0.2 & 0.5 & $3.5\times10^{2}$ & $2.5\times10^{5}$  & $1.2\times10^{-2}$ \\
0.2 & 0.7 & $6.2\times10^{1}$ & $6.4\times10^{3}$  & $2.5\times10^{-2}$ \\
\bottomrule
\end{tabular}
  \begin{tablenotes}
    \footnotesize
    \item Across the whole grid $\mathrm{BF}_\text{joint}$ spans
    $6.4\times10^{3}$ to $2.6\times10^{11}$. \textbf{Every cell exceeds the
    Jeffreys threshold for decisive evidence by at least an order of
    magnitude, and every cell is significant at $\alpha=0.05$}; the
    worst case is $p = 2.5\times10^{-2}$.
    Taking $\rho_w \in [0, 0.2]$ and $\rho_b \in [0.4, 0.5]$ as the central
    range gives $\mathrm{BF}_\text{joint}$ between $2.5\times10^{5}$ and
    $6.0\times10^{7}$. Script: \texttt{sob\_bf\_sensitivity.py}.
  \end{tablenotes}
\end{threeparttable}
\end{table}

The conclusion does not depend on where in this grid one stands. The
independence assumptions govern how strongly the corpus absence should be
quantified, not whether it is present: on every counting scheme, none of the
four features occurs in the Matthean sense anywhere in the corpus.

\paragraph{\grk{Φαίνομαι} in Matthew and the Gospels.}

A separate question concerns the vocabulary Matthew uses of the Star
\emph{outside} 2:9. At 2:7 Herod asks about the time \grk{τοῦ φαινομένου
ἀστέρος}, ``of the appearing star,'' and it might be supposed that this places
the Star in ordinary astronomical description. Matthew's own usage does not
support that supposition. \grk{Φαίνω} occurs eight times in the Gospel:

\begin{center}
\begin{tabular}{@{}llp{6.4cm}@{}}
\toprule
Reference & Form & Subject \\
\midrule
Matt 1:20  & \grk{ἐφάνη}     & \grk{ἄγγελος κυρίου} (an angel of the Lord) \\
Matt 2:7   & \grk{φαινομένου}& \grk{ἀστέρος} (the Star) \\
Matt 2:13  & \grk{φαίνεται}  & \grk{ἄγγελος κυρίου} \\
Matt 2:19  & \grk{φαίνεται}  & \grk{ἄγγελος κυρίου} \\
Matt 9:33  & \grk{ἐφάνη}     & impersonal, of an exorcism: ``never was it so seen in Israel'' \\
Matt 13:26 & \grk{ἐφάνη}     & \grk{τὰ ζιζάνια} (the weeds appeared) \\
Matt 23:27 & \grk{φαίνονται} & whitewashed tombs \emph{appear} beautiful \\
Matt 24:27 & \grk{φαίνεται}  & \grk{ἡ ἀστραπή} (lightning, as the figure of the \grk{παρουσία} of the Son of Man) \\
\bottomrule
\end{tabular}
\end{center}

Three of the eight have an angel as subject, and all three fall in the same two
chapters as the Star. A fourth is the unprecedented manifestation at 9:33 and a
fifth is the lightning of 24:27, which Matthew uses precisely as an image of an
eschatological advent. Only 13:26 and 23:27 are ordinary. In this author,
\grk{φαίνω} is characteristically the verb of manifestation, and most often of
supernatural manifestation.

Matt 24:27 is worth quoting, because it contains both of the terms at issue:
\grk{ὥσπερ γὰρ ἡ ἀστραπὴ ἐξέρχεται ἀπὸ ἀνατολῶν καὶ φαίνεται ἕως δυσμῶν, οὕτως
ἔσται ἡ παρουσία τοῦ υἱοῦ τοῦ ἀνθρώπου}: ``for as the lightning comes out from
the east and shines as far as the west, so will be the coming of the Son of
Man.'' \grk{Ἀνατολή} and \grk{φαίνεται} occur together, in an explicitly
eschatological simile, with no astronomical referent at all.

The wider Gospel usage runs the same way. \grk{Ἐφάνη} is used of the risen
Christ appearing to Mary Magdalene (Mark 16:9) and of Elijah having appeared
(Luke 9:8); \grk{φαίνων} describes John the Baptist as the burning and shining
lamp (John 5:35). The remaining instances are idiomatic uses meaning ``to seem''
(Mark 14:64; Luke 24:11).

\paragraph{What this does and does not establish.}
It does not establish that the Star of 2:7 is a supernatural entity; that is not
the argument. It establishes that \grk{φαινομένου} carries no astronomical
weight, because in Matthew the verb is not astronomical vocabulary but the
vocabulary of appearing, used most often of angels and of the parousia. A term
distributed across both registers cannot discriminate between them, and it is
for that reason, rather than for convenience, that it is not among the four
features tested. The same reasoning applies to \grk{ἐν τῇ ἀνατολῇ}: a heliacal
rising is what Num 24:17, T.~Levi 18:3 and T.~Judah 24:1 predicate of a star
that is a person, and what \textit{Joseph and Aseneth} 14:1 predicates of a star
that is explicitly an angel (\grk{ὁ ἀστὴρ οὗτος ἄγγελος καὶ κῆρυξ}).

\paragraph{Independence of the two analyses.}

$\mathrm{BF}_\text{corpus}$ is computed from the TLG astronomical corpus ($\approx$619{,}000 words
in 24 texts) and the four guiding-star traditions; Matthew's Gospel contributes no
...
\paragraph{The complement of \grk{ἵστημι} (Feature B).}

\grk{Ἵστημι} belongs to the working vocabulary of Greek positional astronomy.
Geminus, Theon of Smyrna, Paul of Alexandria and Hipparchus all use it of a
planet ceasing its motion, and both Theon and Paul build the noun
\grk{στηριγμός} out of the participle and infinitive of \grk{ἵστημι} when
defining it. A writer who wished to say that a planet reached its station
could say so with this verb.

What distinguishes Matthew is the complement. Across 121 in-corpus
occurrences, the thing a celestial body stands in, on, or above is always
celestial or geometric: a zodiacal sign, a degree, an aspect, a chart angle, a
fixed star. The nearest approach to a terrestrial complement is
Cleomedes \textit{Caelestia} 2.1.271, where the sun \grk{στῇ} at exact noon
while Syene is said to lie \emph{under Cancer}, and even there the city is
tied to the sign by \grk{κεῖται}, not to the sun by \grk{ἵστημι}. Matthew's
construction, a star standing \grk{ἐπάνω} the place where a child was, has no
parallel in the corpus, and Matthew's own form \grk{ἐστάθη} does not occur in
it at all.
...
\paragraph{Historical validation of the genre classification.}

The genre classification recovered by both Bayes factors, legendary narrative
rather than astronomical report, is not a product of the statistical method.
No ancient or medieval commentator interpreted the Star's motion in Matt~2:9
as a natural astronomical phenomenon\cite{adair2012}. Patristic and medieval
exegetes universally understood the Star as a miraculous divine guide, not a
planet, comet, or conjunction subject to celestial mechanics. The astronomical
interpretation is a post-Enlightenment reading with no foothold in the
pre-modern exegetical tradition. This is precisely the result the corpus
analysis recovers working from frequency data alone: the vocabulary of Matt~2:9
belongs so consistently to the legendary narrative register that no ancient
reader of Greek, familiar with both registers from the surrounding literature,
ever reached an astronomical interpretation. The statistical method and the
pre-modern exegetical consensus thus converge on the same conclusion by entirely
independent routes, validating that the genre signal is carried by the text
itself, not imposed by the analytical framework.
...
guiding-star traditions), they are treated as conditionally independent of one
another, as they derive from non-overlapping corpora and distinct sampling
frames. Their product is the joint Bayes factor incorporating the linguistic
evidence considered here:
\begin{equation}
  \mathrm{BF}_\text{joint}
    = \mathrm{BF}_\text{Matthew} \times \mathrm{BF}_\text{corpus},
    \qquad \mathrm{BF}_\text{Matthew} = 87{,}696 .
  \label{eq:bf_joint}
\end{equation}
Each factor is itself a product over the same four features, and those features
are four properties of a single clause. The joint factor is therefore reported
as the sensitivity range of Table~\ref{tab:bfsens} rather than as a point
estimate. At the independence corner of that grid
($\rho_w = \rho_b = 0$) it is $87{,}696 \times 3.0\times10^{6} \approx
2.6\times10^{11}$; at the opposite corner ($\rho_w = 0.2$, $\rho_b = 0.7$) it
is $6.4\times10^{3}$. Taking $\rho_w \in [0, 0.2]$ and
$\rho_b \in [0.4, 0.5]$ as the central range gives $\mathrm{BF}_\text{joint}$
between $2.5\times10^{5}$ and $6.0\times10^{7}$. Every cell of the grid
exceeds the Jeffreys threshold for decisive evidence by at least an order of
magnitude.
...
what either could show alone. The combined result is robust to the
feature-independence assumption within each Bayes factor: the copula
sensitivity analysis carries inter-feature correlation as far as
$\rho = 0.95$, and the Fisher combination remains below $\alpha = 0.05$
throughout, while every cell of Table~\ref{tab:bfsens} remains decisive on the
Jeffreys scale.
...
``going before'' a group of travelers. The stopping window itself reaches a
...
east of the corridor for longer. Optimizing over all orbital parameters cannot
...
To assess empirical plausibility I queried the JPL Small Body Database (SBDB),
...
East) described it as exhibiting guiding behavior. This is consistent with the
analysis presented here: the orbital mechanics of close-flyby comets categorically preclude the
...
%%%%  ---------------------------------------------------------------------
\subsection{Analysis of reviewer-proposed cometary orbits}
\label{subsec:r2orbits}
%%%%  ---------------------------------------------------------------------

\paragraph{Background.}
During peer review, a referee proposed two specific cometary orbits as
candidate explanations for the Star of Bethlehem, arguing that bodies on
such trajectories could exhibit behavior consistent with Matthew~2:9.
The orbital elements are reproduced in Table~\ref{tab:r2orbits} for
reference and independent verification.
Orbit~1 shares its angular elements with Matney's 5~BCE comet proposal;
Orbit~2 has a distinct argument of perihelion ($\omega = 220.6361^\circ$)
that places it on a different trajectory.  An earlier draft of this
analysis transcribed Orbit~2's angular elements incorrectly, duplicating
those of Orbit~1; the referee identified the error and supplied the
original configuration, from which the values in
Table~\ref{tab:r2orbits} are taken.

\begin{table}[h]
  \centering
  \caption{Referee-proposed cometary orbital elements (J2000 ecliptic,
  reference JDE~1\,719\,708.5 = 22~April 5~BCE Julian).
  Orbit~2 values are from the referee's corrected Stellarium configuration
  (\textit{orbit\_ArgOfPericenter}, etc.).}
  \label{tab:r2orbits}
  \small
  \begin{tabular}{@{}lrr@{}}
    \toprule
    ...
    \midrule
    Eccentricity $e$ & 0.8070114 & 0.9668455 \\
    Perihelion distance $q$ (AU) & 1.00284101 & 0.9008354 \\
    Perihelion epoch (JDE, TT) & 1\,719\,718.38394 & 1\,719\,735.20246 \\
    Inclination $i$ (°) & 10.9963 & 10.9153 \\
    Ascending node $\Omega$ (°) & 56.1727 & 56.1658 \\
    Arg.\ of perihelion $\omega$ (°) & 193.7693 & 220.6361 \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Method.}
Sky positions were computed with Earth's heliocentric position taken from
VSOP87, which retains full accuracy across antiquity; the bundled modern
DE ephemerides do not extend to 5~BCE.  Orbital elements are referred to
the J2000 ecliptic and equinox, matching the convention of the referee's
Stellarium\cite{stellarium2021} configuration.  Coordinates are carried
explicitly through each frame (J2000 ecliptic, J2000 equatorial,
equinox-of-date equatorial (IAU~1976 precession), then topocentric
alt/az) because at the small geocentric distances involved here both the
precession transformation and the topocentric parallax correction are
first-order effects rather than refinements: for an object near lunar
distance the observer's displacement from Earth's center alone shifts the
apparent position by of order a degree.  Comet epochs are TT and sidereal
time is computed from UT1, with $\mathrm{TT}-\mathrm{UT1}=0.12236$\,d.

The pipeline was validated against the referee's independently generated
Stellarium positions for Orbit~2 at three epochs on 22~April 5~BCE.
Agreement is $2.8^\circ$ RMS in position and ${\approx}7\%$ in geocentric
distance.  This residual is expected rather than anomalous: at a geocentric
distance of ${\approx}2.3\times10^{5}$\,km, the ${\approx}43''$ difference
between independent implementations of Earth's orbit displaces the observer
by ${\approx}3\times10^{4}$\,km, which is itself of order the residual.
Both implementations agree that these objects pass within a lunar distance
of Earth, and agree on the angular rates to well within the margins that
matter for the criteria below.

The observer location was Bethlehem (lat~$31.7^\circ$N, lon~$35.2^\circ$E).
For the night of 22~April 5~BCE (Julian), sunset falls at $18{:}29$ and
sunrise at $05{:}25$ local mean solar time, giving $10.1$\,h of usable
darkness.

Two distinct rate quantities are reported below, and they are not
interchangeable.  The \emph{inertial} rate is motion against the background
stars, the quantity to which the lunar bound of
${\approx}0.5^\circ\,\mathrm{h}^{-1}$ (\S\,2.3) applies, and the sense in
which a star-like object can be said to stand still.  The \emph{ground}
rate is motion relative to the horizon, which includes diurnal rotation;
apparent rest in that frame would require cancelling Earth's
${\approx}15^\circ\,\mathrm{h}^{-1}$ spin.

\paragraph{Orbit~1: results and failure modes.}
Orbit~1 passes within $8.50\times10^{4}$\,km of Earth
($5.68\times10^{-4}$\,AU, $0.22$ lunar distances) shortly after local
midnight on 22~April.  Its inertial angular rate is at a minimum of
$0.93^\circ\,\mathrm{h}^{-1}$ at nightfall, with the comet at
azimuth~$57^\circ$ (ENE) and altitude~$52^\circ$, and rises
\emph{monotonically} thereafter: $2.0^\circ\,\mathrm{h}^{-1}$ two hours
before midnight, $4.8^\circ\,\mathrm{h}^{-1}$ an hour after,
$31.9^\circ\,\mathrm{h}^{-1}$ by the end of the night.  The ground-frame
rate never falls below $7.7^\circ\,\mathrm{h}^{-1}$.  Orbit~1 fails all
three criteria:

\begin{enumerate}
  \item \textit{Stopping.}~The minimum inertial rate,
    $0.93^\circ\,\mathrm{h}^{-1}$, is roughly twice the lunar bound and is
    never approached again; in the ground frame the minimum,
    $7.7^\circ\,\mathrm{h}^{-1}$, exceeds the apparent-rest threshold by a
    factor of~$26$.  Decisively, the object \emph{accelerates} throughout
    the entire night.  The aorist \grk{ἐστάθη} denotes a transition from
    motion to rest, and this trajectory admits no such transition in either
    frame.
  \item \textit{Sequential order.}~The rate minimum occurs at nightfall,
    before any guidance phase could begin, and the motion is monotonically
    increasing from that point on.  The object is at its slowest at the
    start of the night and its fastest at the end, the inverse of the
    required guidance-then-stopping sequence.
  \item \textit{Direction.}~The comet begins the night in the ENE
    (azimuth~$57^\circ$), climbs across the northern sky through the
    meridian region near azimuth~$0^\circ$, and descends into the SW,
    reaching azimuth~$229^\circ$ at the end of the night.  It never enters
    the Jerusalem--Bethlehem corridor $[190^\circ, 210^\circ]$ while
    visible.  A traveler following it would be led north, then west.
\end{enumerate}

\paragraph{Orbit~2: results and failure modes.}
Orbit~2 is the more interesting of the two, and it is the stronger form of
the cometary proposal.  It passes within $2.26\times10^{5}$\,km of Earth
($1.51\times10^{-3}$\,AU, $0.59$ lunar distances) and, unlike Orbit~1, it
does traverse the correct part of the sky: beginning the night in the east
(azimuth~$88^\circ$), it sweeps south and spends ${\approx}1.5$\,h within
the Jerusalem--Bethlehem corridor $[190^\circ, 210^\circ]$ before ending the
night at azimuth~$213^\circ$.  The directional requirement, which excludes
most candidates outright, is therefore satisfied.

It nonetheless fails, and it fails on rate.  The inertial angular rate
begins at $1.5^\circ\,\mathrm{h}^{-1}$ at nightfall and increases
monotonically all night, reaching $15.8^\circ\,\mathrm{h}^{-1}$ by dawn;
while the comet occupies the corridor it is moving at
$6.7$--$10.9^\circ\,\mathrm{h}^{-1}$.  The minimum inertial rate at any
point of visibility is $1.5^\circ\,\mathrm{h}^{-1}$, three times the lunar
bound; the minimum ground-frame rate is $8.4^\circ\,\mathrm{h}^{-1}$,
twenty-eight times the apparent-rest threshold.

\begin{enumerate}
  \item \textit{Stopping.}~The rate never falls below three times the lunar
    bound, and at no point in the night does it decrease at all.  The
    aorist \grk{ἐστάθη} requires a transition from motion to rest; a
    monotonically accelerating object admits no such transition.  This is
    the decisive failure.
  \item \textit{Sequential order.}~As with Orbit~1, the slowest moment is
    the first moment of visibility and the fastest is the last.  Even
    granting the corridor passage as a guidance phase, what follows
    guidance is acceleration, not arrest: the required order is inverted.
  \item \textit{Direction.}~\emph{This criterion is satisfied.}  The comet
    occupies the corridor for approximately $1.5$\,h in the second half of
    the night.  This is recorded explicitly rather than contested.
\end{enumerate}

\paragraph{Summary.}
Orbit~1 fails all three criteria.  Orbit~2 satisfies the directional
criterion and fails on rate: it is never slower than three times the lunar
bound, and it accelerates monotonically from dusk to dawn, so no stopping
event in the sense required by \grk{ἐστάθη} occurs.

That Orbit~2 satisfies the directional criterion is worth stating plainly,
because it locates precisely where the cometary hypothesis is strongest and
where it breaks.  A comet close enough to be conspicuous, and both of these
pass within a lunar distance, is necessarily moving quickly across the sky;
the same proximity that supplies the brightness supplies the angular speed.
The two requirements of the narrative, that the object be seen and that it
be seen to stop, pull in opposite directions.  Direction can be arranged by
choosing the orbit's orientation; rate cannot, because it is fixed by the
geometry that made the object visible in the first place.
These results are consistent with and complementary to the general
nighttime close-flyby survey (\S\,\ref{subsec:closeflyby}):
the present analysis demonstrates that specific representative orbits in
the relevant cometary parameter space fail for concrete, orbit-specific
reasons, not merely because of a broad statistical exclusion.

%%%%  ---------------------------------------------------------------------
\subsection{Survey-scale sweep of the guidance and stopping criteria}
\label{subsec:sweep}
%%%%  ---------------------------------------------------------------------

\paragraph{Purpose.}
A referee proposed that stopping be measured not as an instantaneous angular
rate but as a finite angular \emph{displacement} accumulated over a window, on
the argument that an observer judges rest by comparing positions rather than by
sensing a derivative. That is the better criterion and it is adopted here. The
referee further proposed a threshold in the range $1$--$5^\circ$ per hour. This
section reports the result of applying it, at survey scale and with the
remaining criteria loosened as far as they can be loosened.

\paragraph{Design.}
$10^{9}$ orbits were drawn and propagated, yielding $6{,}116$ close flybys
(minimum geocentric distance below the flyby threshold). Every close flyby was
evaluated on a grid of $525$ parameter combinations: guidance corridor widened
from $20^\circ$ to $60^\circ$ (five values), azimuth-drift tolerance raised from
$1.5$ to $30^\circ\,\mathrm{h}^{-1}$ (seven values), guidance leg shortened from
two hours to fifteen minutes (five values), and stopping window stretched to
eight hours (three values). The whole campaign was repeated under flat priors
(isotropic inclination and log-uniform perihelion distance) as a control on
the stratified sampling used in the main survey.

\paragraph{Two commitments.}
The result rests on exactly two requirements, and everything else is loosened
to the extreme.

\begin{enumerate}
  \item Stopping is a finite angular displacement, not an instantaneous rate.
    This is the referee's own proposal.
  \item The stop must occur at the bearing along which the object guided.
    Matthew has the Star stand over the house to which it has led the Magi, so
    guidance and stopping are linked in azimuth.
\end{enumerate}

Requirement~2 is a \emph{tightening} of the test as originally specified, in
which the two conditions were not linked in azimuth. It is stated here
explicitly, and its effect is given in Table~\ref{tab:corridorlink}, so that
the reader may judge it on the text rather than on the outcome it produces.

\begin{table}[h]
\centering
\begin{threeparttable}
  \caption{Effect of requiring the stop to occur within the guidance
    corridor.}\label{tab:corridorlink}
\begin{tabular}{@{}lcc@{}}
\toprule
Configuration & Stop anywhere & Stop in corridor \\
\midrule
corr.\ $170$--$210^\circ$, drift $15^\circ\,\mathrm{h}^{-1}$, guide $0.25$\,h
  & 1 orbit, min $3.88^\circ$ & \textbf{0}, min $7.81^\circ$ \\
corr.\ $150$--$210^\circ$, drift $30^\circ\,\mathrm{h}^{-1}$, guide $0.25$\,h
  & 1 orbit, min $3.88^\circ$ & \textbf{0}, min $8.08^\circ$ \\
\bottomrule
\end{tabular}
\end{threeparttable}
\end{table}
...
\paragraph{Result.}
Across $10^{9}$ sampled orbits and $6{,}116$ close flybys, no orbit satisfies
the displacement criterion at any of the $525$ parameter combinations tested:
$0$ of $525$ cells. The tightest in-corridor displacement achieved anywhere is
$\mathbf{6.94^\circ}$ per hour, against $5^\circ$ at the loosest end of the
referee's range, a margin of $1.39\times$. Under flat priors the corresponding
figure is $8.71^\circ$, from $450$ close flybys, again with $0$ of $525$ cells
passing. The conclusion is not an artefact of the stratified sampling.

Figure~\ref{fig:sweep} presents the sweep. Panel~(a) is the load-bearing
result: what binds is the azimuth-drift tolerance, not the stopping threshold.
No configuration admits a guidance window at all until the drift tolerance is
raised to $10^\circ\,\mathrm{h}^{-1}$, roughly seven times the published value.
Panel~(b) shows the tightest in-corridor displacement over the surviving
configurations; every populated cell lies between $6.9$ and $8.0^\circ$, and
none approaches the criterion.

\paragraph{The one apparent exception, and why it is not one.}
On the unconstrained metric, stopping permitted at any bearing, exactly one
orbit of the $6{,}116$ reaches $3.88^\circ$, inside the referee's range.
Candidate \#1178 is examined directly in Figure~\ref{fig:falsepos}. It fails on
three independent grounds.

\emph{It is not a comet.} $q = 1.018$\,au, $e = 4.06$, closest approach
$6.2\times10^{5}$\,km ($1.6$ lunar distances), and a hyperbolic excess velocity
of $52\,\mathrm{km\,s^{-1}}$, against $26$ for 1I/ʻOumuamua and $32$ for
2I/Borisov. This is not a Solar System body.

\emph{It does not stop.} The total apparent motion reaches a minimum of
$3.85^\circ\,\mathrm{h}^{-1}$ and never falls below it at any point in the
night. What vanishes is only the altitude component: at the best-displacement
epoch $98\%$ of the residual motion is azimuthal
($|\dot{\mathrm{alt}}| = 0.80$, $|\dot{\mathrm{az}}|\cos\mathrm{alt} = 3.92
^\circ\,\mathrm{h}^{-1}$), and the altitude turning point proper falls half an
hour later still, where the total rate is $3.85^\circ\,\mathrm{h}^{-1}$.

\emph{It is not over the house.} Guidance ends at azimuth $209.5^\circ$; the
minimum occurs at azimuth $233.5^\circ$, $24^\circ$ west of the corridor edge
and $3.8$\,hours later.

A criterion that scores this track as a stop is measuring the moment a
descending object levels out, not a body coming to rest above a place. This is
the diagnostic value of the rate metric, and the reason the displacement
criterion with the corridor link is the one the claim rests on.

\paragraph{The two-leg itinerary, with a turn toward the house.}
A referee further proposed that the Magi's journey need not have held a single
bearing throughout. Taking the Church of the Nativity as a convenient example
of an off-road destination, the final stretch would have required a shift in
azimuth to something nearer $170^\circ$ for the last half hour or so, which
opens up many more permutations of possible Star motion.

Widening the corridor to a single static window does not test that scenario. A
static corridor still demands low azimuth drift throughout, so a star swinging
from $200^\circ$ to $170^\circ$ is excluded however wide the window is made,
while the scenario positively requires that swing. The journey is therefore
modeled here in two legs: the road from Jerusalem, with azimuth held in
$[190^\circ, 210^\circ]$; a free transition of up to an hour in which the
azimuth may swing without constraint; an approach leg with azimuth held in a
second corridor, taken as $[160^\circ,180^\circ]$, $[140^\circ,180^\circ]$ or
$[120^\circ,180^\circ]$ so as to run from due south round to the south-east;
and only then the stopping test. Leg durations were swept at $1.5$, $1.0$ and
$0.5$\,h for the road and $0.5$ and $0.25$\,h for the approach, drift tolerance
within each leg at seven values from $1.5$ to $30^\circ\,\mathrm{h}^{-1}$, and
the stopping threshold at the same six values as above: $756$ combinations,
over the same $6{,}116$ close flybys. This is more permissive than any geometry
tested elsewhere in this work.

\emph{Result.} No parameter set admits a two-leg itinerary. The reason is
directional. The referee's own figure places the final leg near azimuth
$170^\circ$, which lies east of the $190$--$210^\circ$ bearing held along the
road, so the turn requires the Star to swing \emph{eastward}. Diurnal motion
carries objects the other way:

\begin{center}
\begin{tabular}{@{}lc@{}}
\toprule
Orbits whose azimuth increases (westward) through $120$--$210^\circ$ & 2{,}024 \\
Orbits whose azimuth decreases (eastward) & \textbf{0} \\
\midrule
Leg pairs in which the approach leg \emph{precedes} the road leg & 835 \\
Leg pairs in the order the scenario requires & \textbf{0} \\
\bottomrule
\end{tabular}
\end{center}

Where both bearings occur they occur in the reverse order: the Star stands at
$170^\circ$ first and at $200^\circ$ afterwards. An object that turned east to
follow the Magi off the road would have to run against the rotation of the sky
for the last half hour of the journey. The scenario is therefore not merely
unmet but inverted. Script: \texttt{sob\_finalleg\_sweep.py}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig_criteria_sweep}
  \caption{\textbf{Survey-scale sweep of the guidance and stopping criteria.}
    \textbf{(a)}~Orbit-configurations admitting a guidance window, over
    guidance corridor and azimuth-drift tolerance. The drift tolerance is what
    binds: nothing appears until it is raised roughly sevenfold above the
    published value (red line). \textbf{(b)}~Tightest $1$\,h displacement with
    the stop required to fall within the corridor, over drift tolerance and
    guidance duration demanded; every populated cell lies at $6.9$--$8.0^\circ$
    against a $5^\circ$ criterion. \textbf{(c)}~Cumulative distributions of both
    stopping metrics over all close-flyby candidates, showing that the floor is
    a property of the population rather than of a few orbits. \textbf{(d)}~The
    same comparison under stratified and flat sampling priors.}
  \label{fig:sweep}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{fig_false_positive}
  \caption{\textbf{Anatomy of the single orbit passing the unconstrained
    metric.} \textbf{(a)}~The apparent path over Bethlehem. Guidance ends at
    azimuth $209.5^\circ$; the minimum-displacement epoch occurs $24^\circ$
    further west and $3.8$\,h later, well outside the corridor.
    \textbf{(b)}~The two components of the apparent motion. The altitude
    component passes through zero, but the azimuthal component does not, and
    the total apparent motion never falls below
    $3.85^\circ\,\mathrm{h}^{-1}$.}
  \label{fig:falsepos}
\end{figure}
